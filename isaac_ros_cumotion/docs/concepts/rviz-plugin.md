...
| Component | Kind | Role |
|---|---|---|
| `RvizArgsPanel` ("Curobo Rviz Panel") | Panel | Target pose, planner type + planner node selection, parameters, plan/execute buttons |
...
| `TargetDisplay` | Display | Generic 6-DOF target pose (planner/MPC logic lives in the panel) |

### Target pose — the interactive marker

`TargetDisplay` shows a draggable 6-DOF target in the 3D view. The
...
- **Trajectory type** — a combo box for the planner, wired to `set_planner`: Classic, MPC. The indexes map 1:1 to the `SetPlanner` enums (0 = Classic, 1 = MPC).
