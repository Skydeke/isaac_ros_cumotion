...
class ConfigManager:
    ...
    def _load_robot_description(self):
        ...
        if not self.node.has_parameter('robot'):
            self.node.declare_parameter('robot', 'emulator')
        robot = self.node.get_parameter('robot').get_parameter_value().string_value or 'emulator'
    ...
    def _translate_xrdf(self, xrdf_path: str) -> str:
        ...
        in the deployment repo). ``asset_root_path`` defaults to the auto-detected ROS
    ...
    def _detect_asset_root(self) -> str:
        ...
        """
        try:
            # Robot-abstract: anchor on this package's own install share (always
            # present) rather than any robot-specific package, so the detected
            # root is the shared ROS install share/ that contains every package's
            # meshes.
            share = get_package_share_directory('isaac_ros_cumotion')
        except Exception:
            return ''
    ...
    def _load_robot_config(self):
        ...
        if not robot_config_file:
            raise RuntimeError(
                ...
                f"('{self.robot_description.name}') ships without a curobo_config, "
                "and no 'robot_config_file' parameter was provided. Supply the "
                "robot's cuRobo config (and URDF) at launch — deployment repos "
                "(e.g. a kortex package) set robot_config_file/urdf_path.")
