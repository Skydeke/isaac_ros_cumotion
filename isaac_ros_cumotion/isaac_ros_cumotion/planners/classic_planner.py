...
class ClassicPlanner(SinglePlanner):
    ...
    def _plan_trajectory(
        ...
        # (same contract as JointSpace: it applies the goalsets' allowed links
        # around each solve via the set_link_collision service); the planning
        # interface no longer mutates the shared motion-planner collision state.
        try:
            if num_goalset > 1:
