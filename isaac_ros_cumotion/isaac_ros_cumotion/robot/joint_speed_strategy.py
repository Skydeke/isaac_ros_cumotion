...
class JointSpeedStrategy(JointCommandStrategy):
    ...
    def __init__(self, node, dt, description=None):
        ...
        # The driver's publication order on /joint_states may interleave other
        # joints (e.g. a leading finger_joint), so feedback must be remapped by
        # NAME against this list; a plain positional slice would shift every
        # arm joint by one and drop the last one. Captured now because
        # self.joint_names gets overwritten by set_command()/callback below.
    ...
    def callback_joint_pose(self, msg):
        ...
        # when the message names every one of them. This is order-agnostic and
        # drops non-arm joints the driver interleaves (e.g. Kortex publishes
        # finger_joint BEFORE joint_1; a positional slice would shift every arm
        # joint by one and drop joint_7, feeding the planner a garbled — often
        # in-collision — start state). Falls back to a positional read when the
        # names don't line up (some drivers, e.g. /dsr01/joint_states, publish
        # names that need not match the cspace names; curobo only needs the
        # right count/order then).
        ...
        with self.buffer_lock:
            ...
            if remap is not None:
                ...
                if msg.velocity:
                    self.joint_velocity = [msg.velocity[i] for i in remap]
