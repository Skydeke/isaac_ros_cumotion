...
class ESDFVisualizer(Node):
    ...
    def __init__(self):
        ...
        self.__clear_shapes_on_request = (
            self.get_parameter("clear_shapes_on_request")
            .get_parameter_value()
            .bool_value
        )
        self.__clear_shapes_subsampling_factor = (
            self.get_parameter("clear_shapes_subsampling_factor")
            .get_parameter_value()
            .integer_value
        )
        self.__clear_shapes_counter = 0
        self.__aabbs_to_clear_min_m = (
            self.get_parameter("aabbs_to_clear_min_m")
            .get_parameter_value()
            .double_array_value
        )
        self.__aabbs_to_clear_size_m = (
            self.get_parameter("aabbs_to_clear_size_m")
            .get_parameter_value()
            .double_array_value
        )
        self.__spheres_to_clear_center_m = (
            self.get_parameter("spheres_to_clear_center_m")
            .get_parameter_value()
            .double_array_value
        )
        self.__spheres_to_clear_radius_m = (
            self.get_parameter("spheres_to_clear_radius_m")
            .get_parameter_value()
            .double_value
        )
    ...
    def send_request(self, aabb_min_m, aabb_size_m):
        ...
        if (
            self.__clear_shapes_on_request
            and self.__clear_shapes_counter % self.__clear_shapes_subsampling_factor
            == 0
        ):
    ...
    def publish_voxels_from_esdf(self, esdf_data):
        ...
        if occupied_indices.size(0) > self.__max_publish_voxels:
            indices = torch.randperm(occupied_indices.size(0), device=self.__device)[
                : self.__max_publish_voxels
            ]
...
def main(args=None):
    ...
    try:
        ...
        esdf_client.get_logger().info(
            f"Shutting down due to exception of type {type(e)}: {e}"
        )
